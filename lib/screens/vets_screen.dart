  ...
  Future<void> _find() async {
    ...
    try {
      ...
      final items = await _poi.nearby(
        ...
        type: 'vet', radiusM: 30000, limit: 20, nocache: true,
      );
      if (!mounted) return;
      setState(() => _vets = items);
      if (items.isEmpty) ScaffoldMessenger.of(context).showSnackBar(
      ...
      body: GradientWrap(
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                PillButton(icon: Icons.search, label: _busy ? 'Searching…' : 'Find nearby vets', onPressed:_busy?null:_find),
                const SizedBox(width: 10),
                StatChip(icon: Icons.place, label: 'Found', value: '${_vets.length}'),
              ],
            ),
            const SizedBox(height: 12),
            if (_err != null) Text(_err!, style: const TextStyle(color: Colors.white)),
              ...
              child: GlassCard(
                ...
                child: _busy
                  ...
                  itemBuilder: (context, i) {
                    final p = _vets[i];
                    final title = (p.name?.trim().isNotEmpty ?? false) ? p.name! : 'Unnamed clinic';
                    return ListTile(
                      leading: const Icon(Icons.local_hospital, color: Colors.teal),
                      title: Text(title),
                      subtitle: Text('lat ${p.lat.toStringAsFixed(5)}, lon ${p.lon.toStringAsFixed(5)}'),
                      trailing: IconButton(
