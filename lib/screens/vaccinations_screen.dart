...
class _VaccinationsScreenState extends State<VaccinationsScreen> {
  ...
  Future<void> _go() async {
    ...
    try {
      _ai = await ApiService.ai('/ai/vaccinations', breed: _breed.text.trim());
    } catch (e) {
      _err = '$e';
    } finally {
              ...
              child: GlassCard(
                ...
                child: Column(
                  children: [
                    ...
                    // Add row (kept same behavior: submit to add)
                    Padding(
                      padding: const EdgeInsets.fromLTRB(12, 6, 12, 12),
                      child: Row(
                        children: const [
                          Expanded(
                            child: _AddVaccineInlineField(), // below widget
                          ),
                          SizedBox(width: 8),
                          Text('Tap to toggle ✓'),
                        ],
...
// Extracted so behavior stays: submit-to-add using DbService.setVaccination(name,false)
